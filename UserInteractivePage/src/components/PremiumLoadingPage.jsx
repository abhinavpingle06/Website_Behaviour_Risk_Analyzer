...
export default function SecurityDashboard() {
  ...
  const redirectScore = (scanResult?.result?.redirects?.score ?? 0) * 100 ;
  ...
  let overallScore = 0
  if (scanResult?.result?.redirects?.google_result.length > 0) {
    overallScore = 95; // force high risk
    
  } else {
    overallScore = Math.round((contentRisk * 0.3) + (redirectScore * 0.4) + (networkScore * 0.3) + (cookieScore * 0.4));
  }
